...
def lambda_handler(event, context):
    ...
    if output[0]["Object Type"] in ("png", "jpg", "jpeg"):
        ...
        image = Image.open(
            s3.get_object(
                Bucket=output[0]["Bucket Name"], Key=output[0]["Object Name"]
            )["Body"]
        )
        ...
        s3.upload_fileobj(
            in_mem_file,
            Bucket=output[0]["Bucket Name"],
            Key=f'raw_reduced/{dict(output[0]).get("Object Name").split("/")[-1]}',
        )

    sns_client = boto3.client("sns")

    sns_client.publish(
        TopicArn="arn:aws:sns:us-east-1:801355498549:s3_event_notification",
        Message=json.dumps(output[0]),
    )
